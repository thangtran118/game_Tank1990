...
class PlayerTank(pygame.sprite.Sprite):
    ...
    def move(self, direction, scene_elems, player_tanks_group, enemy_tanks_group, home):
        ...
        if self.switch_count > self.switch_time:
            ...
            self.image = self.tank_direction_image.subsurface(
                (48*int(self.switch_pointer), 0), (48, 48))
    ...
    def setDirection(self, direction):
        ...
        if self.direction == 'up':
            self.tank_direction_image = self.tank_image.subsurface(
                (0, 0), (96, 48))
        elif self.direction == 'down':
            self.tank_direction_image = self.tank_image.subsurface(
                (0, 48), (96, 48))
    ...
    def improveTankLevel(self):
        if self.booming_flag:
            return False
        self.tanklevel = min(
            self.tanklevel+1, len(self.player_tank_image_paths)-1)
        self.tank_image = pygame.image.load(
            self.player_tank_image_paths[self.tanklevel]).convert_alpha()
    ...
    def draw(self, screen):
        ...
        if self.is_protected:
            ...
            if self.protected_mask_flash_count > self.protected_mask_flash_time:
                ...
                self.protected_mask_flash_count = 0
            screen.blit(self.protected_mask.subsurface(
                (48*self.protected_mask_pointer, 0), (48, 48)), self.rect)
...
class EnemyTank(pygame.sprite.Sprite):
    def __init__(self, enemy_tank_image_paths, appear_image_path, position, border_len, screensize, bullet_image_paths=None, food_image_paths=None, boom_image_path=None, **kwargs):
        ...
        self.appear_images = [appear_image.subsurface((0, 0), (48, 48)), appear_image.subsurface(
            (48, 0), (48, 48)), appear_image.subsurface((96, 0), (48, 48))]
        ...
        if (random.random() >= 0.6) and (self.tanklevel == len(self.enemy_tank_image_paths)-2):
            self.tanklevel += 1
            self.food = Foods(food_image_paths=food_image_paths,
        ...
        self.move_cache_count = 0
        self.tank_image = pygame.image.load(
            self.enemy_tank_image_paths[self.tanklevel]).convert_alpha()
